...
func newTestCheckFromSpec(c *kubernetes.Clientset, spec *khcheckv1.KuberhealthyCheck) *external.Checker {
	...
	checker := external.New(c, spec, khCheckClient, khStateClient, nil, cfg.ExternalCheckReportingURL) // external checker does not ever return an error so we drop it
	checker.Debug = true
	return checker
}
